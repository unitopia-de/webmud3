...
export class GmcpService {
  ...
  private find_menu(mud_id:string,actual_menu : GmcpMenu) : number {
    ...
    for (var i = 0; i < this.gmcpmenus[mud_id].length;i++) {
  ...
  public menuAction(actual_menu : GmcpMenu) {
    ...
    var other = this;
    actual_menu.cfg.callback(actual_menu.action,actual_menu.cfg,actual_menu,index,function(reaction){
      switch (reaction) {
        case 'toggle': 
          if (index > -1) {
  ...
  public set_gmcp_support(mud_id:string,gmcp_support:Object,cb_mudSwitchGmcpModule) {
    ...
    this.unitopiaSrv.set_callbacks(mud_id,
  ...
  private find(mud_id:string,module_name:string):number {
    for (var i = 0; i < this.gmcpconfig.length;i++) {
  ...
  public remove_gmcp_module(mud_id:string,module_name:string) : boolean {
    var index = this.find(mud_id,module_name);
    if (index>=0) {
