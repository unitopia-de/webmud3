...
| LINEMODE                                  | Full           | WILL               | DO                 | Client negotiates LINEMODE with MODE (EDIT/TRAPSIG/SOFT_TAB), acknowledges SLC changes, publishes a CR/LF forward mask so lines flush only on Enter, and surfaces EDIT toggles so the frontend can fall back to character mode.                                         | https://github.com/unitopia-de/webmud3/issues/114 |
